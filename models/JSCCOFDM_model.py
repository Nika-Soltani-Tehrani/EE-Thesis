...
class JSCCOFDMModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netE = (
            ...
            .define_E(input_nc=3,
                      ngf=64,
                      max_ngf=256,
                      n_down_sample=opt.n_downsample,
    ...
    def forward(self, cof_in=None):
        ...
        if self.opt.feedforward == 'OFDM-feedback':
            H_true = H_true.permute(0, 1, 3, 2).contiguous().view(N, -1, latent.shape[2], latent.shape[3]).to(
                latent.device)
        ...
        # The receiver contains CE and EQ modules plus the CE and EQ subnet
            ...
            sub2_input = (
                torch
                .cat((sub21.repeat(1, 1, self.opt.S, 1, 1), sub22, sub23), 3)
                .reshape(-1, 1, 3, self.opt.M, 2)
                .contiguous()
                .permute(0, 1, 2, 4, 3)
                .contiguous()
                .view(-1, 6, 8, 8)
    ...
    def backward_D(self):
        ...
        if self.opt.gan_mode in ['lsgan', 'vanilla']:
            ...
            penalty, grad = (
                networks.cal_gradient_penalty(self.netD, 
                                              real_data, 
                                              self.fake.detach(), 
                                              self.device, 
                                              type='mixed', 
    ...
    def backward_G(self):
        ...
        if self.opt.feedforward in ['OFDM-CE-sub-EQ', 'OFDM-CE-sub-EQ-sub']:
            # Channel estimations are complex numbers
            self.loss_G_H = self.opt.lam_h * torch.mean((self.H_est - self.H_true) ** 2) * 2 
        else:
            self.loss_G_H = 0
