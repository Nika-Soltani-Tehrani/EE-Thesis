...
def define_E(input_nc, ngf, max_ngf, n_downsample, C_channel, n_blocks, norm='instance', init_type='kaiming',
             init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    net = Encoder(input_nc=input_nc, ngf=ngf, max_ngf=max_ngf, C_channel=C_channel, n_blocks=n_blocks,
                  n_downsampling=n_downsample, norm_layer=norm_layer, padding_type="reflect")
    return init_net(net, init_type, init_gain, gpu_ids)


def define_G(output_nc, ngf, max_ngf, n_downsample, C_channel, n_blocks, norm="instance", init_type='kaiming',
             init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    net = Generator(output_nc=output_nc, ngf=ngf, max_ngf=max_ngf, C_channel=C_channel, n_blocks=n_blocks,
                    n_downsampling=n_downsample, norm_layer=norm_layer, padding_type="reflect")
    return init_net(net, init_type, init_gain, gpu_ids)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_nc, ngf=64, max_ngf=512, C_channel=16, n_blocks=2, n_downsampling=2,
        ...
        for i in range(n_blocks):
            model += [ResnetBlock(min(ngf * mult, max_ngf), padding_type=padding_type, norm_layer=norm_layer,
    ...
    def forward(self, input_bit_stream, H=None):

        z = self.model_down(input_bit_stream)
        z = self.model_res(z)
...
class Generator(nn.Module):
    def __init__(self, output_nc, ngf=64, max_ngf=512, C_channel=16, n_blocks=2, n_downsampling=2,
        ...
        Parameters:
            ...
            ngf (int)           -- the number of filters in the first conv layer
            max_ngf (int)       -- the maximum number of filters
            C_channel (int)     -- the number of channels of the input_bit_stream
            n_blocks (int)      -- the number of ResNet blocks
            n_downsampling      -- number of downsampling layers
            norm_layer          -- normalization layer
            padding_type (str)  -- the name of padding layer in conv layers: reflect | replicate | zero
    ...
    def forward(self, input_bit_stream):
        ...
        return 2 * self.model(input_bit_stream) - 1
...
class Subnet(nn.Module):
    ...
    def build_conv_block(self, dim, dim_out, dim_in, padding_type, norm_layer, use_dropout, use_bias):
        conv_block = []
        conv_block += [nn.Conv2d(dim, dim_in, kernel_size=5, padding=2, bias=use_bias), norm_layer(dim_in),
                       nn.ReLU(True)]
        conv_block += [nn.Conv2d(dim_in, dim_in, kernel_size=5, padding=2, bias=use_bias), norm_layer(dim_in),
                       nn.ReLU(True)]
