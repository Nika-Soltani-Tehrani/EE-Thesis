...
#           C_channel / (3 x 2^(2 x n_down_sample + 1))
...
if __C.dataset_mode in ['CIFAR10', 'CIFAR100']:
    ...
    __C.n_downsample = 2  # Down sample times
    __C.n_blocks = 2  # Number of residual blocks


elif __C.dataset_mode == 'CelebA':
    __C.n_layers_D = 3  # Number of layers in the discriminator. Only used with GAN loss
    __C.n_downsample = 3  # Down sample times
    __C.n_blocks = 2  # Number of residual blocks
    ...
    __C.n_downsample = 3  # Down sample times
    __C.n_blocks = 4  # Number of residual blocks
